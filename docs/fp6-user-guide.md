...
#   - Quant:   SparkInfer MX-FP6, W6A6 (6-bit weights, 6-bit activations).
#              quantization_config = {"quant_method":"modelopt","quant_algo":"W6A6"}
...
Same usage pattern. Note: the "FP6 FusedMoE binding is TP=1-only" comment
inside the script is stale — MoE expert sharding works through vLLM's
loader, and TP=2 has been validated (the script adds
`--disable-custom-all-reduce` automatically at TP>1). At single-stream
loads TP=2 buys capacity, not latency.
...
# Single GPU -> no tensor parallelism. NOTE: the FP6 FusedMoE binding is
