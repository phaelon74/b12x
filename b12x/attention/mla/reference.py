...
def unpack_mla_kv_cache_fp6_qk_via_fp8_bytes_reference(
    kv_cache: torch.Tensor,
    *,
    group_size: int = _MLA_GROUP_SIZE,
) -> torch.Tensor:
    """Unpack K for ``B12X_MLA_DEBUG_QK_BF16=1`` (FP8 decode on raw nope bytes * scale)."""
...
def sparse_mla_fp6_debug_qk_bf16_reference(
    *,
    q_all: torch.Tensor,
    kv_cache: torch.Tensor,
    page_table_1: torch.Tensor,
    ...
    """Reference for ``B12X_MLA_DEBUG_QK_BF16=1``: BF16 QK via FP8-on-bytes K, FP6-dequant V."""

    k_all = unpack_mla_kv_cache_fp6_qk_via_fp8_bytes_reference(kv_cache).squeeze(1).to(
        torch.float32
    )
    v_all = unpack_mla_kv_cache_fp6_reference(kv_cache, fmt=fmt).squeeze(1).to(torch.float32)
    return _sparse_attention_reference(
        q_all=q_all,
        k_all=k_all,
        v_all=v_all[:, :v_head_dim],
        page_table_1=page_table_1,
        active_token_counts=active_token_counts,
        sm_scale=sm_scale,
        return_lse=return_lse,
