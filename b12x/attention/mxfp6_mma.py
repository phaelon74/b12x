...
def _literal_pv_mma_into_ofrag_mxfp6_scaled_mla(
    ...
    """MLA sparse PV MMA with MX-FP6 V (paged FP6 K=32 layout: 2 mma_kv interleaved per A reg)."""
    ...
    for mma_pair in cutlass.range_constexpr(_MLA_NUM_MMA_KV // 2):
        mma_kv0 = mma_pair * 2
        mma_kv1 = mma_kv0 + 1
        if cutlass.const_expr(mma_pair == 0):
            scale01_a = scale01_k0
            scale89_a = scale89_k0
            scale01_b = scale01_k1
            scale89_b = scale89_k1
        else:
            scale01_a = scale01_k2
            scale89_a = scale89_k2
            scale01_b = scale01_k3
            scale89_b = scale89_k3

        # Per-token scaled P (KV dequant scale folded into P before FP6 quantize).
        pa0 = bfloat2_mul(p_frag[0, mma_kv0, 0], scale01_a)
        pa1 = bfloat2_mul(p_frag[0, mma_kv0, 1], scale01_a)
        pa2 = bfloat2_mul(p_frag[0, mma_kv0, 2], scale89_a)
        pa3 = bfloat2_mul(p_frag[0, mma_kv0, 3], scale89_a)
        pb0 = bfloat2_mul(p_frag[0, mma_kv1, 0], scale01_b)
        pb1 = bfloat2_mul(p_frag[0, mma_kv1, 1], scale01_b)
        pb2 = bfloat2_mul(p_frag[0, mma_kv1, 2], scale89_b)
        pb3 = bfloat2_mul(p_frag[0, mma_kv1, 3], scale89_b)

        # Dynamic UE8M0 scale over the full K=32 block so tiny folded P doesn't underflow FP6.
        hmax_local = bfloat2_hmax_to_f32(
            bfloat2_hmax2(
                bfloat2_hmax2(
                    bfloat2_hmax2(bfloat2_habs2(pa0), bfloat2_habs2(pa1)),
                    bfloat2_hmax2(bfloat2_habs2(pa2), bfloat2_habs2(pa3)),
                ),
                bfloat2_hmax2(
                    bfloat2_hmax2(bfloat2_habs2(pb0), bfloat2_habs2(pb1)),
                    bfloat2_hmax2(bfloat2_habs2(pb2), bfloat2_habs2(pb3)),
                ),
            )
        )
        # The m16n8k32 block-scale operand (bid_a=tid_a=0) broadcasts a single
        # lane's sfa byte across the MMA rows, so a per-lane-varying sfa would be
        # misapplied to rows owned by other lanes. Reduce hmax to one warp-wide
        # max so every row uses an identical scale (correct regardless of the
        # scale-operand row/thread selection).
        hmax = hmax_local
        for shift in cutlass.range_constexpr(5):
            hmax = fmax_f32(hmax, cute.arch.shuffle_sync_bfly(hmax, offset=1 << shift))
        sfa0 = cvt_f32_to_ue8m0(hmax)
        # Shift the UE8M0 exponent down by the FP6 format's max exponent so the
        # block max maps to ~2^EMAX (near the format's max representable) instead
        # of ~1.0, keeping P in FP6's high-precision region. The shift is applied
        # to both `inv_sfa` and the `sfa` MMA operand, so it cancels exactly.
        if cutlass.const_expr(kv_dtype == cutlass.Float6E3M2FN):
            sfa0 = sfa0 - Uint32(4)
        else:
            sfa0 = sfa0 - Uint32(2)
        inv_sfa = broadcast_f32_to_bfloat2(ue8m0_to_output_scale(sfa0))
        sfa = Uint32(sfa0) | (Uint32(sfa0) << Uint32(8)) | (
            Uint32(sfa0) << Uint32(16)
        ) | (Uint32(sfa0) << Uint32(24))

        pa0 = bfloat2_mul(pa0, inv_sfa)
        pa1 = bfloat2_mul(pa1, inv_sfa)
        pa2 = bfloat2_mul(pa2, inv_sfa)
        pa3 = bfloat2_mul(pa3, inv_sfa)
        pb0 = bfloat2_mul(pb0, inv_sfa)
        pb1 = bfloat2_mul(pb1, inv_sfa)
        pb2 = bfloat2_mul(pb2, inv_sfa)
        pb3 = bfloat2_mul(pb3, inv_sfa)

        # K=32 A-fragment layout (matches verified `_literal_qk_mma_into_sfrag_mxfp6_raw_mla`):
        # the two K-halves occupy separate register PAIRS, not low/high of the same reg.
        #   a_regs[0,0..1] = kv0 (K 0-15), packing reg0|reg2 and reg1|reg3
        #   a_regs[0,2..3] = kv1 (K 16-31)
        a_regs = cute.make_rmem_tensor(
            cute.make_layout((1, 4), stride=(4, 1)),
            Uint32,
        )
        if cutlass.const_expr(kv_dtype == cutlass.Float6E3M2FN):
            a_regs[0, 0] = (cvt_bf16x2_to_e3m2x2(pa0) & mask16) | (
                (cvt_bf16x2_to_e3m2x2(pa2) & mask16) << shift16
            )
            a_regs[0, 1] = (cvt_bf16x2_to_e3m2x2(pa1) & mask16) | (
                (cvt_bf16x2_to_e3m2x2(pa3) & mask16) << shift16
            )
            a_regs[0, 2] = (cvt_bf16x2_to_e3m2x2(pb0) & mask16) | (
                (cvt_bf16x2_to_e3m2x2(pb2) & mask16) << shift16
            )
            a_regs[0, 3] = (cvt_bf16x2_to_e3m2x2(pb1) & mask16) | (
                (cvt_bf16x2_to_e3m2x2(pb3) & mask16) << shift16
            )
        else:
            a_regs[0, 0] = (cvt_bf16x2_to_e2m3x2(pa0) & mask16) | (
                (cvt_bf16x2_to_e2m3x2(pa2) & mask16) << shift16
            )
            a_regs[0, 1] = (cvt_bf16x2_to_e2m3x2(pa1) & mask16) | (
                (cvt_bf16x2_to_e2m3x2(pa3) & mask16) << shift16
            )
            a_regs[0, 2] = (cvt_bf16x2_to_e2m3x2(pb0) & mask16) | (
                (cvt_bf16x2_to_e2m3x2(pb2) & mask16) << shift16
            )
            a_regs[0, 3] = (cvt_bf16x2_to_e2m3x2(pb1) & mask16) | (
                (cvt_bf16x2_to_e2m3x2(pb3) & mask16) << shift16
            )

        v_offset_k0 = v_offset
        v_offset_k1 = _advance_offset_by_row_128b(
            v_offset, 16, Int32(_MLA_NOPE_GROUP_KV_VECS)
        )
        for mma_d in cutlass.range_constexpr(_MLA_VO_NUM_MMA_D):
            if cutlass.const_expr(mma_d % 2 == 0):
                b0_k0, b1_k0 = ldmatrix_m8n8x4_trans_left_half_b16(
                    _smem_addr_from_b128_offset(v_base_addr, v_offset_k0)
                )
                b0_k1, b1_k1 = ldmatrix_m8n8x4_trans_left_half_b16(
            ...
            o_frag[0, mma_d, 6] = d6
            o_frag[0, mma_d, 7] = d7
            if cutlass.const_expr(mma_d % 2 == 1):
                v_offset_k0 = _advance_offset_by_column_128b_2(v_offset_k0, mma_d // 2)
                v_offset_k1 = _advance_offset_by_column_128b_2(v_offset_k1, mma_d // 2)

        v_offset = _advance_offset_by_row_128b(
            v_offset, 32, Int32(_MLA_NOPE_GROUP_KV_VECS)
        )
