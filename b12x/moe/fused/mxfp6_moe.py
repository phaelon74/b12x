...
def moe_emit_mma_k_block(
    ...
    ``fmt_a`` / ``fmt_b`` accept either FP6 sub-format strings (``"e3m2"`` /
    ``"e2m3"`` — the migrated byte-container static path) or, for the not-yet-
    migrated dynamic backend, the legacy ``Float6E3M2FN`` / ``Float6E2M3FN``
    dtypes; both emit the inline ``mxf8f6f4`` MMA. ``None`` or a non-FP6 dtype
    falls back to the native FP4 block-scaled MMA. The migrated FP6 operands are
    carried in ``Float8E4M3FN`` byte-containers, indistinguishable from FP8 by
    dtype, so the FP6 sub-format must be passed explicitly.
