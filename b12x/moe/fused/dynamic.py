...
class MoEDynamicKernelBackend:
    ...
    def __init__(
        ...
        *,
        ...
        # ``mxf8f6f4`` FP6 instruction (cutlass has no working 6-bit smem
        # layout). Mirrors the static byte-container backend.
    ...
    def _setup_attributes(self):
        ...
        if cutlass.const_expr(self.mxfp6_fmt_a is not None):
            ...
            # warp_n_base), so the atom layout must be (2,2,1) = 4 atom positions.
            # (4,2,1) describes 8 atom positions (used WITH 8 warps in the dense
            # path); with only 4 warps it halves each thread's accumulator so only
            # half the N columns of FC1/FC2 are computed. num_m_tiles must then be
            # M//(16*2) so num_m_tiles*atom_m*16 == tile_M. Mirrors the static fix.
        ...
        permutation_mnk = sm120_utils.get_permutation_mnk(
            self.tile_shape_mnk, self.sf_vec_size, use_perm_k,
        )
        ...
        # MMA dispatch selector: FP6 byte-containers are indistinguishable from
        # FP8 by dtype, so pass the explicit sub-format string ("e3m2"/"e2m3");
        # the native FP4 path keeps passing the operand dtype.
        self._mma_fmt_a = (
            self.mxfp6_fmt_a if self.mxfp6_fmt_a is not None else self.a_dtype
        )
        self._mma_fmt_b = (
            self.mxfp6_fmt_b if self.mxfp6_fmt_b is not None else self.b_dtype
        )
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.mxfp6_fmt_a is not None):
            ...
            # Byte-container scratch: one FP6 code per byte (32 bytes/block,
            # K bytes/row) so the Float8E4M3FN A TMA/ldmatrix path consumes it.
                    ...
                    if token_idx < num_tokens:
                        ...
                        if lane_id == Int32(0):
                            ...
                            while topk_slot < num_topk:
                                ...
                                # Deterministic mode records the routed PAIR
                                # index so the host combine can rebuild a
                                # canonical (token, slot) order — physical row
                                # assignment races permute rows run-to-run.
                                if cutlass.const_expr(self.deterministic_scatter):
                                    st_global_i32(get_ptr_as_int64(token_map, phys_row), pair_idx)
                                else:
        ...
        while consumer_live > Int32(0):
            ...
            if has_task > Int32(0):
                ...
                task_slice_begin_cache = _ld_shared_i32(ctrl_base_addr + Int32(16))
                ...
                while cache_row < Int32(self.tile_shape_mnk[0]):
                    ...
                    if cache_row < task_valid_rows_cache:
                        ...
                        # Deterministic mode scatters each (physical row,
                        # intermediate slice) partial to its own unique staging
                        # row: FC2 adds one PARTIAL contribution per slice
                        # (tasks are one slice each, _TASK_SLICE_CHUNK == 1),
                        # so rows must be split per slice — exactly one atomic
                        # add per location keeps the (unchanged) scatter_add
                        # epilogue bit-exact. Host sums slices then topk slots
                        # in canonical order.
                        if cutlass.const_expr(self.deterministic_scatter):
                            tok = (
                                global_row_cache * route_gate_tile_cnt
                                + task_slice_begin_cache
                            )
                        else:
            ...
            if has_task == Int32(0):
                ...
                while slice_idx < task_slice_count_val:
                    ...
                    if cutlass.const_expr(self.mxfp6_fmt_a is not None):
                        # Byte-container A smem: one FP6 code per byte, K bytes
                        # per row (matches the static byte-container requant).
                    ...
                    for epi_m in cutlass.range_constexpr(epi_rest_m):
                        ...
                        if epi_rows < Int32(0):
                            epi_rows = Int32(0)
                        if cutlass.const_expr(self.mxfp6_fmt_a is not None):
                            # Zero the FP6 intermediate SFA + A-code smem before the
                            # FC2 requant store. The store only covers
                            # epi_rows*sf_blocks; any padding-row slot the vec32 MMA
                            # reads but the store misses would otherwise hold stale
                            # UE8M0 bytes -> 2^huge -> inf blowup (UE8M0 does not
                            # tolerate stale; ue=0 => 2^-127 ~ 0). Mirrors static.
                        ...
                        while quant_idx < epi_rows * sf_blocks_per_row:
                            ...
                            if cutlass.const_expr(self.mxfp6_fmt_a is not None):
                                ...
                                # Byte-container swizzled store into the Float8 A
                                # smem (8-bit K-major SW128 atom Sw<3,4,3>): for
                                # col<K the physical offset is flat ^ ((row&7)<<4).
                                # Mirrors the static byte-container requant; the
                                # prior recast/packed write was un-swizzled and
                                # only correct at row 0.
