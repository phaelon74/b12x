...
# --- MX-FP6 (W6A6) decode micro path --------------------------------------
# MX-FP6 weights use 32-element UE8M0 block scales (vs FP4's 16-element E4M3),
# and are stored 3:4-packed (3 bytes per 4 codes). The decode micro path mirrors
# the FP4 micro GEMV but resolves codes through the shared decode LUT primitive
# (b12x.cute.fp6.fp6_decode_*) instead of the FP4 software dot.
...
def _fp6_micro_enabled() -> bool:
    ...
    Keeps the validated static/dynamic FP6 fused path as the only FP6 backend
    until the micro kernel is wired into dispatch and numerically proven.
    """
...
def _mxfp6_micro_shapes_ok(
    ...
    Mirrors ``MoEMicroKernelBackend.is_supported`` but at the FP6 32-element block
    granularity. FC1-chunk feasibility for a specific ``(m, n)`` is validated by
    ``configure`` once the kernel geometry is built; this gate covers the
    constraints that hold independent of the kernel inner-loop tiling.
    """
    if m not in (1, 2, 4, 8):
        return False
...
class MoEMicroKernelBackend:
    ...
    def is_supported_mxfp6(
        ...
        """MX-FP6 (W6A6) decode micro path support.

        Opt-in via ``B12X_ENABLE_FP6_MICRO`` (default off) so the validated
        static/dynamic FP6 fused path stays the only FP6 backend until the micro
        kernel is wired into dispatch and numerically proven. When enabled,
        returns the format-fundamental shape predicate.
        """
