...
block-scale layout that :func:`b12x.integration.tp_moe.b12x_moe_fp6` consumes.
...
@dataclass
class FP6MoEWeights:
    """Quantized MX-FP6 MoE weights ready to pass to ``b12x_moe_fp6``."""
    ...
    *,
    ...
    Args:
        w1_bf16: FC1 weights ``(E, 2*N, K)`` (gate+up stacked along rows, in the
            same row order ``b12x_moe_fp6`` expects). For ReLU2 (non-gated) this is
            ``(E, N, K)``.
    ...
    Returns:
        :class:`FP6MoEWeights` with everything needed to call ``b12x_moe_fp6``.
