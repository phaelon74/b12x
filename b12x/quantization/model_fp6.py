...
``model.safetensors.index.json``), discovers the weights the b12x FP6 kernels
can consume, quantizes them with the offline primitives, and writes per-layer
artifacts + a ``manifest.json``.

* :func:`discover_moe_experts` / :func:`convert_moe_model_to_fp6` - routed-expert
  FFNs -> packed MX-FP6 in the layout ``b12x_moe_fp6`` expects. FC1 rows are
  stacked ``[up; gate]`` (the kernel's gated-silu convention, verified by
...
def convert_moe_model_to_fp6(
    ...
    *,
    ...
    """
    from b12x.quantization import quantize_moe_weights_to_fp6, save_fp6_moe_weights
...
def convert_dense_model_to_fp6(
    ...
    *,
    ...
    from b12x.quantization import quantize_dense_weight_to_fp6
