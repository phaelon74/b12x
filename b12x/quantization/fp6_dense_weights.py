...
This is the dense counterpart of :mod:`b12x.quantization.fp6_moe_weights`. It
targets ordinary ``nn.Linear`` weights (MLP gate/up/down, attention q/k/v/o
projections) and the block-scaled :func:`b12x.gemm.dense.dense_gemm` kernel,
rather than fused-MoE experts.
...
from b12x.gemm.dense import dense_gemm, _DENSE_FUSED_QUANT
...
# output buffers belong. Kill-switch for A/B runs: B12X_DENSE_PERSISTENT_SCRATCH=0.
# B12X_DENSE_PER_ROW_GS=0 disables per-row activation global scale (m>1 unfused
...
def _small_m_quant_scratch(m_pad: int, k: int, device: torch.device) -> tuple:
    ...
    """
    from b12x.quantization import allocate_bf16_to_fp6_tma_outputs
...
def _quantize_matrix_fp6_bytes_small_m(
    ...
    """
    from b12x.quantization.bf16_to_fp6_small_m import compile_bf16_to_fp6_small_m

    m, k = mat_bf16.shape
    launch = compile_bf16_to_fp6_small_m(m, k, fmt=fmt)
    ...
    out, alpha = _small_m_quant_scratch(m_pad, k, mat_bf16.device)
    launch(
        mat_bf16.contiguous(),
        w_global_scale,
        out.packed_a_flat,
    ...
    return out.packed_a_storage.view(m_pad, k), out.scale_storage, alpha
...
class FP6DenseWeight:
    ...
    def expanded_weight(self) -> torch.Tensor:
        ...
        if self._packed_expanded is None:
            # Lazy import: avoids any import cycle with ``b12x.gemm.dense``.
            from b12x.gemm.dense import _expand_packed_mxfp6_ab
...
def quantize_dense_weight_to_fp6(
    ...
    *,
    ...
    from b12x.quantization.fp6_checkpoint import activation_format_for_source
    ...
    *,
    ...
    if _fused_quant:
        ...
        if _PERSISTENT_SCRATCH:
            _scratch, _ = _small_m_quant_scratch(m_pad, k, device)
