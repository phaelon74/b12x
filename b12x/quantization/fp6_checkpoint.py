...
This module defines the *on-disk* contract for a B12X FP6 checkpoint and the
low-level primitive that quantizes a single linear weight into that contract.
It is deliberately framework-agnostic: it produces plain CPU tensors + a
``quantization_config`` dict, so the model-level exporter
(:mod:`b12x.quantization.model_fp6`) and any serving adapter (vLLM / SGLang)
...
and ``quant_algo="W6A6"`` so a framework can route FP6 weights to the B12X kernel.
"""
...
SCHEMA_VERSION = "b12x_fp6_safetensors_v1"
...
def build_quantization_config(
    *,
    ...
    can detect the scheme, while carrying the extra MX-FP6 specifics B12X needs.
...
def _pack_codes(mat_bf16: torch.Tensor, fmt: str, *, use_gpu: bool) -> torch.Tensor:
    ...
    from b12x.cute.fp6 import quantize_grouped_mxfp6_torch
...
def _fp6_round_tables(
    ...
    from b12x.cute.fp6 import _decode_fp6_e2m3, _decode_fp6_e3m2
...
def _pack_codes_and_scales_mse(
    ...
    """
    from b12x.cute.fp6 import pack_fp6_codes_tensor
...
def dequantize_linear_from_fp6(
    ...
    *,
    ...
    """
    from b12x.cute.fp6 import expand_mxfp6_packed_to_bytes
