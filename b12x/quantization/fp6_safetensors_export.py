...
Unlike the per-layer ``.pt`` converters in :mod:`b12x.quantization.model_fp6`
(useful for kernel validation), this writes a *complete, loadable* model
directory: sharded ``model-*.safetensors`` + ``model.safetensors.index.json`` +
a ``config.json`` patched with a ModelOpt-mirror ``quantization_config``, plus
all the original auxiliary files (tokenizer, generation config, ...).

Quantized linears follow the :mod:`b12x.quantization.fp6_checkpoint` schema
...
from b12x.quantization.model_fp6 import (
    SafetensorsModel,
    _split_gate_up,
...
def dequantize_fp6_checkpoint_to_bf16(
    ...
    *,
    ...
    The output runs on stock vLLM with no B12X involvement, so a KLD against
    the original BF16 model isolates *weight* quantization error from the
    runtime W6A6 *activation* quantization error.
    """
    model = SafetensorsModel(model_path)
