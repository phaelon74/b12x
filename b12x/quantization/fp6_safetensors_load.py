...
Read side of :mod:`b12x.quantization.fp6_safetensors_export`. Given the
per-expert ModelOpt-mirror keys (``.weight`` / ``.weight_scale`` /
``.weight_scale_2`` / ``.input_scale``), this reconstructs the exact
:class:`~b12x.quantization.fp6_moe_weights.FP6MoEWeights` layout that
``b12x_moe_fp6`` consumes:
...
def load_fp6_moe_checkpoint(
    ...
    *,
    ...
    """
    from b12x.quantization.model_fp6 import SafetensorsModel, discover_moe_experts
