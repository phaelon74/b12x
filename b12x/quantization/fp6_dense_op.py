...
Registers ``b12x::fp6_dense_linear`` so vLLM's ``torch.compile`` path (Dynamo
fullgraph tracing + CUDA-graph capture) treats the whole FP6 linear — activation
quantization, CUTE kernel JIT lookup, and the block-scaled GEMM launch — as a
single opaque node instead of tracing into the JIT machinery (tempfile /
os.getpid), which previously forced ``--enforce-eager``.
...
from b12x.quantization.fp6_dense_weights import dense_fp6_linear_expanded
