#!/usr/bin/env python
"""Microbenchmark: sparkinfer W6A6 FP6 fused MoE vs a BF16 grouped-MoE baseline.

Times the FP6 MoE launch (historically ``b12x_moe_fp6``; pending a
``sparkinfer.moe.fused_moe`` rewrite) across token counts that exercise both the static
...
This is a *latency* microbench (correctness is covered by
tests/quantization/test_fp6_gpu.py).
...
def _backend(num_tokens: int, num_topk: int) -> str:
    # TODO(port): select_tp_moe_backend came from b12x.integration.tp_moe, which
    # has no upstream sparkinfer equivalent; rewrite against
    # sparkinfer.moe.fused_moe plan/bind/run once the w6a8_mx run path lands.
    from b12x.integration.tp_moe import select_tp_moe_backend  # historical b12x reference
...
def main() -> None:
    ...
    # TODO(port): the timed launch below drove the historical b12x.integration.tp_moe
    # host API (allocate_tp_moe_workspace / b12x_moe_fp6 / clear_tp_moe_caches).
    # Rewrite against the sparkinfer.moe.fused_moe plan/bind/run flow once the
    # w6a8_mx run path lands upstream; the code is kept below as reference.
